...
def render():
    ...
    if top_rules:
        ...
        for rank, r in enumerate(top_rules, 1):
            medal = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"][rank - 1]
            hit   = r.get("HIT_CNT", 0)
            label = f"{medal} **{r['RULE_ID']}** — {hit}회 사용"
            with st.expander(label, expanded=(rank == 1)):
                st.write(f"**type:** `{r.get('RULE_TYPE', 'SEARCH')}`")
                st.write("**guidance:**")
                for line in (r.get("GUIDANCE") or "").splitlines():
                    if line.strip():
                        st.write(f"- {line.strip()}")
                if r.get("EXAMPLE_BAD_SQL"):
                    st.write("**example_bad_sql:**")
                    st.code(r["EXAMPLE_BAD_SQL"], language="sql")
                if r.get("EXAMPLE_TUNED_SQL"):
                    st.write("**example_tuned_sql:**")
                    st.code(r["EXAMPLE_TUNED_SQL"], language="sql")
        st.divider()
