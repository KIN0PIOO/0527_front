...
from server.tools.context import callbacks, mig_registry, sql_registry, tuning_registry
...
def build_poll_jobs_tool(
    ...
    def poll_jobs() -> str:
        ...
        반환값: migration_jobs, sql_jobs, tuning_jobs 목록과 summary를 담은 JSON 문자열."""
        ...
        try:
            ...
            if logger:
                logger.error(f"[poll_jobs] SQL/Tuning 조회 오류: {exc}")
        ...
        if logger:
            s = result["summary"]
            if s["migration_total"] or s["sql_total"] or s["tuning_total"]:
                logger.info(
                    f"[poll_jobs] "
